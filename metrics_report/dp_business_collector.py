...
accessed via their Iceberg mirror views: iceberg_db_views.cosmos_db__public__<table>.
...
INNER JOIN iceberg_db_views.cosmos_db__public__dataset_dataset AS d ON d.name = f.tbl_name AND d.is_active = TRUE
...
WITH dataset AS (
    ...
    FROM iceberg_db_views.cosmos_db__public__dataset_dataset d
...
SELECT d.name
FROM iceberg_db_views.cosmos_db__public__dataset_dataset d
JOIN iceberg_db_views.cosmos_db__public__features_hivesummary h ON h.dataset_id = d.id
JOIN iceberg_db_views.cosmos_db__public__features_mirrortable m ON m.cdc_dataset_id = d.id
...
SELECT d.name
FROM iceberg_db_views.cosmos_db__public__dataset_dataset d
JOIN iceberg_db_views.cosmos_db__public__dataset_datasetbaserefresh dr ON dr.dataset_id = d.id
...
SELECT d.name
FROM iceberg_db_views.cosmos_db__public__features_validation f
JOIN iceberg_db_views.cosmos_db__public__features_mirrortable m ON m.id = f.mirror_table_id
JOIN iceberg_db_views.cosmos_db__public__dataset_dataset d       ON d.id = m.cdc_dataset_id
...
WITH latest_run AS (
    SELECT run_version
    FROM iceberg_db_views.cosmos_db__public__cdc_table_validation_stats
    ORDER BY id DESC LIMIT 1
)
SELECT DISTINCT d.name
FROM iceberg_db_views.cosmos_db__public__dataset_dataset d
LEFT JOIN iceberg_db_views.cosmos_db__public__cdc_table_validation_stats_last_passed s
    ON s.dataset_id = d.id
LEFT JOIN iceberg_db_views.cosmos_db__public__cdc_table_validation_stats st
